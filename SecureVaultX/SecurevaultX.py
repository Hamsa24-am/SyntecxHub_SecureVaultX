"""
SecureVault X - Premium Enterprise-Grade Cybersecurity Platform
...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        if "encrypted_filename" not in existing_columns:
            cursor.execute(
                "ALTER TABLE files ADD COLUMN encrypted_filename TEXT DEFAULT ''"
            )
    ...
    def add_file(self, user_id, filename, encrypted_filename, size):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO files (user_id, filename, encrypted_filename, size, modified_at) VALUES (?, ?, ?, ?, ?)",
            (user_id, filename, encrypted_filename, size, current_timestamp()),
        )
        conn.commit()
        conn.close()
...
class NeonGlowingButton(ctk.CTkButton):
    """Animated neon button with pulsing glow effect"""
    def __init__(self, parent, primary=False, secondary=False, **kwargs):
        if primary:
            color = PRIMARY
            hover_color = PRIMARY_GLOW
        elif secondary:
            color = SECONDARY
...
class AnimatedNeonLabel(ctk.CTkLabel):
    """Label with animated neon glow effect"""
    def __init__(self, parent, text="", glow=False, **kwargs):
        color = PRIMARY_GLOW if glow else TEXT_PRIMARY
        defaults = {
            "text": text,
            "text_color": color,
...
class NeonProgressWindow:
    """Beautiful animated neon progress window"""
    def __init__(self, title, message, parent=None):
        self.window = ctk.CTkToplevel(parent)
        self.window.title(title)
        self.window.geometry("450x180")
        self.window.configure(fg_color=DEEP_DARK)
        self.window.grab_set()
        
        # Main container
        container = ctk.CTkFrame(self.window, fg_color=DEEP_DARK)
        container.pack(fill="both", expand=True, padx=30, pady=30)
        
        # Neon header
        header = ctk.CTkLabel(container, text=title, text_color=PRIMARY_GLOW,
                             font=("Segoe UI", 14, "bold"))
        header.pack(anchor="w", pady=(0, 15))
        
        # Status message
        self.status_label = ctk.CTkLabel(container, text=message, text_color=TEXT_SECONDARY,
                                        font=("Segoe UI", 11))
        self.status_label.pack(anchor="w", pady=(0, 10))
        
        # Neon progress bar
        progress_frame = ctk.CTkFrame(container, fg_color=CARD_DARKER, corner_radius=8)
        progress_frame.pack(fill="x", pady=(0, 10), ipady=2)
        
        self.progress_bar = ctk.CTkProgressBar(progress_frame, height=6, progress_color=ACCENT_GLOW)
        self.progress_bar.pack(fill="x", padx=2, pady=2)
        self.progress_bar.set(0)
        
        # Percentage
        self.percent_label = ctk.CTkLabel(container, text="0%", text_color=ACCENT_GLOW,
...
class PremiumDashboardPage:
    ...
    def load_files_content(self):
        ...
        # === TOOLBAR SECTION WITH NEON EFFECTS ===
        toolbar_card = PremiumFrame(main_container, fg_color=CARD_DARK)
        ...
        # Left side - Neon Title with glow
        title_frame = ctk.CTkFrame(toolbar_inner, fg_color=CARD_DARK)
        title_frame.pack(side="left", fill="x", expand=True)
        neon_title = ctk.CTkLabel(title_frame, text="📁 My Files", text_color=ACCENT_GLOW,
                                 font=("Segoe UI", 20, "bold"))
        neon_title.pack(anchor="w")
        
        # Right side - Action buttons with neon effects
        ...
        upload_btn = NeonGlowingButton(actions_frame, text="📤 Upload & Encrypt", command=open_upload_dialog, 
        ...
        refresh_btn = NeonGlowingButton(actions_frame, text="🔄 Refresh", command=refresh_files, 
                                       secondary=True, height=40, width=120)
        ...
        search_entry = ctk.CTkEntry(actions_frame, textvariable=search_var, placeholder_text="Search files...",
                                    width=180, height=40, corner_radius=10, fg_color=CARD_DARKER,
                                    border_color=ACCENT_GLOW, border_width=2, text_color=TEXT_PRIMARY,
        ...
        # Drag-and-drop zone with neon effects
        dnd_card = PremiumFrame(main_container, fg_color=CARD_DARKER)
        dnd_card.pack(fill="x", padx=30, pady=(0, 20))
        
        dnd_inner = ctk.CTkFrame(dnd_card, fg_color=CARD_DARK)
        dnd_inner.pack(fill="x", padx=20, pady=20)
        
        dnd_label = ctk.CTkLabel(dnd_inner, text="🎯 Drag & drop files here to encrypt and upload", 
                                text_color=ACCENT_GLOW, font=("Segoe UI", 11, "bold"))
        dnd_label.pack(pady=10)
        
        dnd_sub = ctk.CTkLabel(dnd_inner, text="or use the Upload button above",
                              text_color=TEXT_MUTED, font=("Segoe UI", 9))
        dnd_sub.pack()
        ...
        # Table header with neon styling
        header_card = PremiumFrame(scroll_container, fg_color=CARD_DARKER)
        ...
        # Headers with proper widths and neon colors
        headers_info = [
            ("File Name", 0.25),
            ("Size", 0.15),
            ("Date Modified", 0.20),
    ...
    def _create_file_row(self, parent, filename, enc_filename, size, modified_at):
        ...
        # Size
        size_label = PremiumLabel(row_inner, text=format_size(size), style="secondary")
        ...
        def decrypt_action():
            try:
                # Create animated progress window
                progress = NeonProgressWindow("🔓 Decrypting File", "Preparing decryption...", self.app)
                self.app.update_idletasks()
                
                # Step 1: Ensure encrypted file exists locally
                progress.update_progress(10, "Checking encrypted file...")
                try:
                    self._ensure_encrypted_file_local(enc_filename)
                except Exception as e:
                    progress.close()
                ...
                progress.update_progress(40, "Decrypting with AES-128-CBC...")
                try:
                    ...
                    messagebox.showerror("Decryption Failed", 
                                       ...
                                       f"File may be corrupted or key mismatch.")
                    self.logger.log(f"Decryption Error: {filename} - {str(e)}")
                    return
                
                # Step 3: Verify integrity
                progress.update_progress(80, "Verifying file integrity...")
                try:
                    verified, msg = self.encryption_manager.verify_integrity(enc_filename)
                    if not verified:
                        progress.close()
                        messagebox.showwarning("Integrity Warning", 
                                             f"⚠ File integrity check failed:\n{msg}\n\n"
                                             f"The file may have been modified.")
                        self.logger.log(f"Integrity Check Failed: {filename}")
                        return
                except Exception:
                    pass  # Continue even if integrity check fails
                
                # Step 4: Complete
                progress.update_progress(100, "✓ Decryption complete!")
                ...
                messagebox.showinfo("✓ Success", 
                                   ...
                                   f"📁 Location: {decrypted_path}\n\n"
                                   f"Use Download button to save to custom location")
                ...
                messagebox.showerror("Decryption Error", 
                                   f"Unexpected error during decryption:\n\n{str(exc)}")
        
        decrypt_btn = NeonGlowingButton(actions_frame, text="🔓 Decrypt", command=decrypt_action, 
                                       primary=True, width=80, height=35)
        decrypt_btn.pack(side="left", padx=2)
        
        # Verify button
        def verify_action():
            try:
                progress = NeonProgressWindow("✓ Verifying Integrity", "Calculating file hash...")
                
                progress.update_progress(20, "Checking encrypted file...")
                self._ensure_encrypted_file_local(enc_filename)
                
                progress.update_progress(40, "Decrypting file for hash...")
                verified, message = self.encryption_manager.verify_integrity(enc_filename)
                progress.update_progress(80, "Comparing integrity values...")
                progress.update_progress(100, "✓ Verification complete!")
                ...
                if verified:
                    messagebox.showinfo("✓ Integrity Verified", 
                                       f"✓ {message}\n\n"
                                       f"🔐 File is authentic and unmodified\n"
                                       f"Hash matches stored HMAC-SHA256")
                else:
                    messagebox.showwarning("⚠ Integrity Check Failed", 
                                         f"⚠ {message}\n\n"
                                         f"🚨 File may have been tampered with!\n"
                                         f"Hash does not match stored value")
            except Exception as exc:
                try:
                    progress.close()
                except:
                    pass
                self.logger.log(f"Verification Error: {filename} - {str(exc)}")
                messagebox.showerror("Verification Error", f"Error: {str(exc)}")
        
        verify_btn = NeonGlowingButton(actions_frame, text="✓ Verify", command=verify_action,
                                      secondary=True, width=80, height=35)
        verify_btn.pack(side="left", padx=2)
        
        # Download button
        def download_action():
            try:
                progress = NeonProgressWindow("⬇ Downloading File", "Preparing decryption...")
                
                progress.update_progress(20, "Checking encrypted file...")
                self._ensure_encrypted_file_local(enc_filename)
                
                progress.update_progress(25, "Decrypting file...")
                decrypted_path = self.encryption_manager.decrypt_file(enc_filename)
                progress.update_progress(50, "Opening save dialog...")
                progress.close()
                ...
                if save_path:
                    progress = NeonProgressWindow("⬇ Saving File", f"Saving to:\n{save_path}")
                    import shutil
                    shutil.copy2(decrypted_path, save_path)
                    progress.update_progress(100, "✓ File saved successfully!")
                ...
                try:
                    ...
                    pass
                self.logger.log(f"Download Failed: {filename} - {str(exc)}")
                messagebox.showerror("Download Error", f"Error: {str(exc)}")
        
        download_btn = NeonGlowingButton(actions_frame, text="⬇ Download", command=download_action,
                                        width=90, height=35)
        ...
        def delete_action():
            if not messagebox.askyesno("Confirm Delete", f"🚨 Permanently delete '{filename}'?\n\nThis cannot be undone."):
                return
            try:
                progress = NeonProgressWindow("🗑 Deleting File", "Removing from server...")
                progress.update_progress(30, "Removing from server...")
                self._delete_from_server(enc_filename)
                progress.update_progress(60, "Removing from database...")
                self.db.delete_file_record(self.user_id, enc_filename)
                progress.update_progress(80, "Removing local copy...")
                local_path = os.path.join("encrypted", enc_filename)
                if os.path.exists(local_path):
                    os.remove(local_path)
                progress.update_progress(100, "✓ File deleted successfully!")
                ...
                try:
                    ...
                    pass
                self.logger.log(f"Delete Failed: {filename} - {str(exc)}")
                messagebox.showerror("Delete Error", f"Error: {str(exc)}")
        
        delete_btn = NeonGlowingButton(actions_frame, text="🗑 Delete", command=delete_action,
                                      width=80, height=35)
        delete_btn.pack(side="left", padx=2)

    def _handle_file_upload(self, file_path):
        """Handle file encryption and upload with beautiful neon progress"""
        try:
            filename = os.path.basename(file_path)
            
            # Create neon progress window
            progress = NeonProgressWindow("📤 Uploading & Encrypting", f"File: {filename}")
            
            def progress_callback(percent):
                progress.update_progress(int(percent * 0.5), f"Encrypting... {percent}%")
            ...
            def upload_chunk_callback(encrypted_filename, encrypted_chunk, chunk_index, total_chunks):
                try:
                    ...
                    # Update UI with upload progress (50-100%)
                    upload_progress = int(50 + ((chunk_index + 1) * 50 / total_chunks))
                    progress.update_progress(upload_progress, f"Uploading... {upload_progress}%")
                except Exception as e:
                    raise ConnectionError(f"Upload failed: {str(e)}")
            
            # Step 1: Encrypt
            progress.update_progress(5, "Starting encryption process...")
            self.app.update_idletasks()
            ...
            progress.update_progress(90, "Saving file metadata...")
            self.db.add_file(self.user_id, filename, encrypted_filename, size)
            self.logger.log(f"File Uploaded & Encrypted: {filename} ({format_size(size)})")
            
            # Step 3: Complete
            progress.update_progress(100, "✓ Upload complete!")
            progress.close()
            ...
            messagebox.showerror("Upload Error", f"Error during upload:\n\n{str(exc)}")
...
© 2024 SecureVault X | All Rights Reserved"""
    ...
    def _ensure_encrypted_file_local(self, encrypted_filename):
        local_path = os.path.join("encrypted", encrypted_filename)
        if not os.path.exists(local_path):
            self._download_from_server(encrypted_filename)
        return local_path
